...
public class TicketService {
    ...
    public int getSoldTicketsForDistribution(int eventId,int distributorId) {
        int n = 0;
        for (SoldTickets soldTickets: findAll()) {
            if(soldTickets.getDistributor().getDistributorId() == distributorId  &&  soldTickets.getSeats().getEvent().getEventId() == eventId){
                n += soldTickets.getNumberOfTickets();
            }
        }
        return n;
    }
